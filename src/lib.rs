...
fn register_constants(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    for (k, v) in interface::constants::data_channel_type() {
    ...
    for (k, v) in interface::constants::rtc_topic() {
    ...
    for (k, v) in interface::constants::sport_cmd() {
    ...
    for (k, v) in interface::constants::audio_api() {
    ...
    for (k, v) in interface::constants::app_error_messages() {
...
fn unitree_webrtc_rs(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<interface::py_api::PyUnitreeWebRTCConnection>()?;
    m.add_class::<interface::py_api::PyWebRTCConnectionMethod>()?;
    m.add_class::<interface::py_api::PyVuiColor>()?;
    register_constants(_py, m)?;

    let driver_module = PyModule::new(_py, "webrtc_driver")?;
    driver_module.add_class::<interface::py_api::PyUnitreeWebRTCConnection>()?;
    driver_module.add_class::<interface::py_api::PyWebRTCConnectionMethod>()?;
    driver_module.add_class::<interface::py_api::PyVuiColor>()?;
